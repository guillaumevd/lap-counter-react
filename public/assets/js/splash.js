...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "LAP-COUNTER V2 !", "author": "Guillaume" }
        ]
    ...
    ipcRenderer.invoke('fetch', 'http://146.59.227.84:3000/streaming/manifest.json').then(res => {
    ...
    startLauncher() {
        this.setStatus(`Starting of the program`);
        ipcRenderer.send('main-window-open');
        ipcRenderer.send('update-window-close');
    }

    shutdown(text) {
        this.setStatus(`${text}<br>Stoping in 5s`);
        let i = 4;
        setInterval(() => {
            this.setStatus(`${text}<br>Stoping in ${i--}s`);
